...
// ──────────────────────────────────────────────────────────
// 1. Budget Insights (Overspending + Burn Rate)
// ──────────────────────────────────────────────────────────
export async function calculateBudgetInsights(
  userId: string,
  date: Date
  ...
  // Fetch actual monthly expense total
  ...
  // Alert if spending is significantly ahead of calendar progress
...
// ──────────────────────────────────────────────────────────
// 2. Trend Insights ("What Changed?" report)
// ──────────────────────────────────────────────────────────
export async function calculateTrendInsights(
  userId: string,
  date: Date
): Promise<InsightRecord[]> {
  ...
  const prevMap = new Map(
    prevGroups.map((g) => [g.category, g._sum.amount?.toNumber() || 0])
  );
  const currMap = new Map(
    currentGroups.map((g) => [g.category, g._sum.amount?.toNumber() || 0])
  );

  const insights: InsightRecord[] = [];
  const allCategories = new Set([...prevMap.keys(), ...currMap.keys()]);

  // Per-category trend shifts
  const changes: {
    category: string;
    prev: number;
    curr: number;
  ...
  for (const cat of allCategories) {
    ...
    if (Math.abs(diff) > 200) {
      const pct =
        prev === 0 ? 100 : Number(((diff / prev) * 100).toFixed(1));
      changes.push({ category: cat, prev, curr, diff, pct });
    }
  }

  // Sort by absolute difference descending, take top 3 shifts
  ...
  if (totalPrev > 0 && Math.abs(overallDiff) > 500) {
    const pct = Number(
      ((overallDiff / totalPrev) * 100).toFixed(1)
    );
...
// ──────────────────────────────────────────────────────────
// 3. End-of-Month Prediction
// ──────────────────────────────────────────────────────────
export async function calculatePredictionInsight(
  userId: string,
  date: Date
  ...
  // Need at least 5 days of data for a meaningful prediction
...
// ──────────────────────────────────────────────────────────
// 4. Generate and persist all insights for a user
// ──────────────────────────────────────────────────────────
export async function generateAllInsights(
  ...
  // Collect all insight records in parallel
  ...
  if (prediction) allInsights.push(prediction);

  // Clear old insights for this month before writing new ones
  await prisma.spendingInsight.deleteMany({
    where: { userId, month: monthStart },
  });

  // Bulk insert all computed insights
